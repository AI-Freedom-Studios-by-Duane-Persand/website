...
**Note:** Ayrshare JWT generation requires Max Pack subscription ($200+/month). For a free alternative, use Meta (Facebook/Instagram) API integration instead.
...
#### Social Account OAuth Connection Flow (Business Plan - Requires Max Pack)
...
**Step 2: Generate JWT (Backend - REQUIRES MAX PACK)**
```typescript
...
**One-Step Alternative (Recommended - Requires Max Pack):**
```typescript
...
- **Ayrshare Basic:** $9-25/month (single-user multi-platform posting)
- **Ayrshare Business Plan:** Pricing on request (multi-user)
- **Ayrshare Max Pack:** Additional $200+/month (required for JWT generation)

**Alternative:** Use Meta (Facebook/Instagram) API integration instead (free with Meta API limits)
...
2. **Connect Social Accounts:**
   - If using Business Plan with Max Pack: Generate JWT via `POST /api/social-accounts/connect/jwt-new`
   - Otherwise: Manually connect accounts in Ayrshare dashboard
...
- [Max Pack Information](../AYRSHARE_MAX_PACK_REQUIRED.md)
